...
if(R_COMMAND)
  ...
  execute_process(
    COMMAND ${R_COMMAND} "--slave" "--vanilla" "-e" "cat( .libPaths()[1L])"
              OUTPUT_VARIABLE R_LIB_PATHS
  )
  string(REGEX REPLACE "/00LOCK-cpptorch.*$" "" R_LIB_PATHS "${R_LIB_PATHS}")
  message(STATUS "Assume Rcpp and testthat is in '${R_LIB_PATHS}'")

  # find the packages that we need to link with
  file(GLOB_RECURSE RCPP_HINT "${R_LIB_PATHS}/*/Rcpp.h")
  message(status "${RCPP_HINT}")
  string(REGEX REPLACE "/Rcpp.h$" "" RCPP_HINT "${RCPP_HINT}")
  find_path(Rcpp_INCLUDE_DIR "Rcpp.h" HINTS "${RCPP_HINT}")

  if(NOT Rcpp_INCLUDE_DIR)
    message(FATAL_ERROR "Did not find Rcpp files")
  endif()
  message(STATUS "Found Rcpp include files at '${Rcpp_INCLUDE_DIR}'")

  set(TESTTHAT_HINT "${R_LIB_PATHS}/testthat/include/testthat.h")
  if(NOT EXISTS ${TESTTHAT_HINT})
    message(FATAL_ERROR "Did not find testthat at ${TESTTHAT_HINT}")
  endif()

  string(REGEX REPLACE "/testthat.h$" "" TESTTHAT_HINT "${TESTTHAT_HINT}")
  find_path(testthat_INCLUDE_DIR "testthat.h" HINTS "${TESTTHAT_HINT}")

  if(NOT testthat_INCLUDE_DIR)
    message(FATAL_ERROR "Did not find testthat files")
  endif()
  message(STATUS "Found testthat include files at '${testthat_INCLUDE_DIR}'\n")
